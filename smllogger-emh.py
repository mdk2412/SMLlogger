...
broker='10.9.11.60'
strom_rrd = "%s/strom-emh.rrd" % (os.path.dirname(os.path.abspath(__file__)))
...
# kleine Schleife, um sich auf erste Hälfte der Start- bzw. Stopp-Sequenz zu synchronisieren
	...
	while True:
		try:
			while True:
				...
				if data_raw == '\x1b\x1b\x1b\x1b\x01\x01\x01\x01' >= 0:
					data_raw += ser.read(752) # Nachrichtenlänge 760 Zeichen
					reading_ok = True
					break
		...
		if reading_ok:
			...
			if crc_rx == crc_calc:
				sml180 = parseSML(data_hex, '070100010800ff', 48, 16) # Wirkenergie in Wh (total)
				sml280 = parseSML(data_hex, '070100020800ff', 42, 16)  # Wirkenergie in Wh (Einspeisung)
				sml167 = parseSML(data_hex, '070100100700ff', 42, 8)  # Wirkleistung in W
				sml367 = parseSML(data_hex, '070100240700ff', 42, 8)  # Wirkleistung L1 in W
				sml567 = parseSML(data_hex, '070100380700ff', 42, 8)  # Wirkleistung L2 in W
				sml767 = parseSML(data_hex, '0701004c0700ff', 42, 8)  # Wirkleistung L3 in W
                                sml5171 = parseSML(data_hex, '070100510701ff', 42, 8)
                                sml5172 = parseSML(data_hex, '070100510702ff', 42, 8)
                                sml5174 = parseSML(data_hex, '070100510704ff', 42, 8)
                                sml517f = parseSML(data_hex, '07010051070fff', 42, 8)
                                sml517a = parseSML(data_hex, '07010051071aff', 42, 8)
				msgs = [{'topic':"stromzaehler/bezug", 'payload':(sml180/10000.0000)},
					("stromzaehler/leistung", float(sml167)),
					("stromzaehler/leistung-L1", float(sml367)),
					("stromzaehler/leistung-L2", float(sml567)),
					("stromzaehler/leistung-L3", float(sml767)),
					("stromzaehler/einspeisung", sml280/10000.0000),
                                        ("stromzaehler/5171", sml5171),
                                        ("stromzaehler/5172", sml5172),
                                        ("stromzaehler/5174", sml5174),
                                        ("stromzaehler/517f", sml517f),
                                        ("stromzaehler/517a", sml517a),]
				try:
